...
thawPull :: (Type a, Shapely sh, ShapeTup sh) => (Tuple (ShapeTupT sh), Data [a]) -> DPull sh a
...
instance (Syntax a, Shapely sh, ShapeTup sh, SyntacticTup (ShapeTupT sh)) => Syntactic (Manifest sh a) where
...
manifestToArr :: (Syntax a, ShapeTup sh) => Manifest sh a -> (Tuple (ShapeTupT sh), Data [Internal a])
...
arrToManifest :: (Syntax a, Shapely sh, ShapeTup sh) => (Tuple (ShapeTupT sh), Data [Internal a]) -> Manifest sh a
